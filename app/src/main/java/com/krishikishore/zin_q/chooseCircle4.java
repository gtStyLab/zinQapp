...
public class chooseCircle4 extends AppCompatActivity {

    private static int RESULT_LOAD_IMAGE = 1;
    private static final int REQUEST_IMAGE_ALBUM = 2;
    private static final int MY_PERMISSIONS_REQUEST_READ_EXTERNAL_STORAGE = 1;
    private static final String TAG = "chooseCircle4";
    ...
    chooseCircle4.ScreenResolution deviceDimensions() {
        int currentapiVersion = android.os.Build.VERSION.SDK_INT;
        // getsize() is available from API 13
        if (currentapiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB_MR2) {
            Display display = chooseCircle4.this.getWindowManager().getDefaultDisplay();
            android.graphics.Point size = new Point();
            display.getSize(size);
            return new chooseCircle4.ScreenResolution(size.x, size.y);
        }
        else {
            Display display = chooseCircle4.this.getWindowManager().getDefaultDisplay();
            // getWidth() & getHeight() are deprecated
            return new chooseCircle4.ScreenResolution(display.getWidth(), display.getHeight());
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_choose_circle4);
        try {
            ...
            chooseCircle4.ScreenResolution screenRes = deviceDimensions();
